...
short int findreg()
{
	...
	scanname();
	for (i = 0; i < 12; i++) {
		if (*regtable[i].name == *namebuf) return regtable[i].value;
	}
	return 0;
}
skipspace()
...
scanoperands()
{
	...
	switch (c) {
	case 'A': case 'B': case 'C': case 'D': case 'E': case 'F': case 'G': case 'H': case 'I': case 'J': case 'X': case 'Y':
		reg = findreg();
		break;
	case ',':
		srcptr++;
...
processline()
{
	...
	lp = 0;
	codeptr = 0;
	if (isalnum(*srcptr)) {	//this is used with labels
		scanname(); lp = findsym(namebuf);
		if (*srcptr == ':') srcptr++;
	}
	skipspace();	//removes tab effectively
	if (isalnum(*srcptr)) {	//works on instruction
		scanname();	//seperates opecode from operand, converts lower case to upper case
		op = findop(namebuf);	//binary search for instruction
		if (op) {
			if (op->cat != 13) {	//if not pseudo opp
				setlabel(lp);
				generating = 1;
			}
			co = op->code;
			switch (op->cat) {		//how big opcode, operand, mode
			case 0:onebyte(co); break;	//like lsra with no additional parsing needed. 
...
processfile(char *name)
{
	...
	while (!terminate&&fgets(inpline, 128, srcfile)) {
		expandline(); //from inpline to namebuf
		lineno++;
		srcptr = srcline;
